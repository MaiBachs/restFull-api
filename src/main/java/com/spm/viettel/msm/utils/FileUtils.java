...
public class FileUtils {
    ...
    private static String STR_NAME_VALID = "ABCDEFGHIJKMNLOPQRSTUVWXYZabcdefghijkmnlopqrstuvwxyz0123456789_.- ";
...
//                line = Native2Ascii.asciiToNative(line);
    ...
    public static boolean isFileNameValid(String fileName) {
        ...
        for (int i = 0; i < fileName.length(); i++) {
            ...
            if (c == '/' || c == '\\' || STR_NAME_VALID.indexOf(c) == -1) {
                return false;
            }
        }
        return true;
    ...
    /*
     *  Des: ham lay ten file chuan an toan
     * @author: QuanT
     */
    ...
    public static String getSafeFileName(String fileName) {
        ...
        if (fileName != null) {
            for (int i = 0; i < fileName.length(); i++) {
                char c = fileName.charAt(i);
                if (c != '/' && c != '\\' && c != 0 && STR_NAME_VALID.indexOf(c) > -1) {
                    rslt.append(c);
                }
            }
        }
    ...
    /*
     *  Des: ham lay ten  duogn dan an toan
     * @author: QuanT
     */
    public static String getSafePath(String pathName) {        
        boolean check = true;
        String rslt = pathName;
        do {
            if (rslt != null && !rslt.equals("") && rslt.startsWith(".")) {
    ...
    public static String concatPath(String... paths) {
        if (paths.length == 0) {
            ...
            for (int i = 1; i < paths.length; ++i) {
                result = result.concat((result.endsWith("/") ? "" : "/").concat(paths[i]));
            }
